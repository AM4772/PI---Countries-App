  ...
  switch (type) {
    ...
    case "FILTER_BY_CONTINENT": // filter countries by continent
      ...
      const continentFiltered =
        payload === "All"
          ? allCountries
          : allCountries.filter((t) => t.continent === payload);
      return {
        ...state,
    ...
    case "FILTER_BY_ACTIVITY": // filter countries by activity
      ...
      // console.log(all);
      const allFilteredByAct =
        payload === "all-activities"
          ? all.filter((a) => a.activities.length > 0)
          : all.filter(
              (t) =>
                t.activities &&
                t.activities.map((b) => b.name).includes(payload)
            );
    ...
    case "SORT_BY_NAME":
      const sortOrder =
        payload === "A-Z"
          ? state.countryActivities.sort((a, b) => a.name.localeCompare(b.name))
          : state.countryActivities.sort((a, b) =>
    ...
    case "SORT_BY_POPULATION":
      const sortPopulation =
        payload === "Low"
          ? state.countryActivities.sort((a, b) => a.population - b.population)
          : state.countryActivities.sort((a, b) => b.population - a.population);
      return {
        ...state,
